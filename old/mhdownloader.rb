...
def download_one(manga, chapter, use_multithreading)
	manga.include?("-mh") ? manga_final = manga[0, manga.index("-mh")] : manga_final = manga
	...
	pages.each do |page|
		if use_multithreading
			threads << Thread.new do
				url = "#{MHDIR}/#{manga_final}/#{chapter}/#{page}" #http://img.mangahost.net/br/mangas_files//tamen-de-gushi/1/02.jpg
				Downloader.new(url, page)
			end
		else
			url = "#{MHDIR}/#{manga_final}/#{chapter}/#{page}"
			Downloader.new(url, page)
		end
