# require 'shoes'
#This interface currently does work
exit
@@mhd = Mangahost.new("", "", "")
Shoes.app(:title => "Manga Host Downloader #{$VERSION}", width: 570, height: 310, resizable: false) do
	...
	flow do
		stack(:margin => 10, :width => 350) do  
			...
			flow do
				para("Chapter \t")
				@edit_chapter = edit_line(state: "disabled", width: 200) {edit_Change}
			end
	...
	def interface_status(state)
		@edit_name.state = @edit_chapter.state = @search_manga.state = @download.state = state
	end
	...
	def search_manga_Click
		...
		@edit_name.state = @search_manga.state = @download.state = "disabled"

		exits = MDownloader::Mangahost.url_page_exits?("mangahost.me", "/manga/#{@edit_name.text}/")		
		if exits
			@@mhd.manga_name = @edit_name.text
			# link = @@mhd.get_cover
			# @manga_thumb.path = link
			@thumb_alt.text = @edit_name.text.gsub("_", " ")
			@edit_chapter.state = @search_manga.state = @download.state = @edit_name.state = nil
		else 
	...
	def download_Click
		...
		@@mhd.path_to_download = @edit_path.text
		@@mhd.manga_chapter = @edit_chapter.text
